...
- **KOReader Sync**: Options for setting up KOReader for syncing book progress. **Smart sync** is the default for new configurations and auto-resolves simple push/pull decisions. Existing credential files retain **Ask every time** when migrated; you can switch Sync Behavior at any time if you prefer manual confirmation. **Match Other Copies** is off by default: it names each book by its layout as well as by its file, so a re-encoded or re-downloaded copy can find the place another copy stored. Only a server carrying [koreader-sync-server#55](https://github.com/koreader/koreader-sync-server/pull/55) reads those names; anywhere else the requests are unchanged. A copy with the same chapters, such as one run through the EPUB Optimizer, opens at the exact place. A copy whose chapters differ is placed by percentage, and the sync screen labels it **Approximate: from another copy**.
